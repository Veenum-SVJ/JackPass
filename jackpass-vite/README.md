...
A production-ready migration of the Next.js 15 JackPass app to **Vite 5 + React 18 + React Router v6 + TanStack Query v5**.

JackPass is an academic question bank platform for Nigerian students: search, view, and solve past questions from Nigerian universities and institutions.
...
│   ├── hooks/               # TanStack Query hooks + useMediaQuery/use-toast
│   ├── lib/                 # supabase clients, paystack, ocr, subscription, mappers
│   ├── pages/               # Route components (Home, Login, Profile, admin/*, ...)
│   ├── styles/              # globals.css (Tailwind + design tokens)
│   ├── test/                # Vitest setup
│   ├── App.tsx              # Providers + route table
│   └── main.tsx             # Entry (QueryClient + BrowserRouter)
└── server/                  # Express API server
    ├── routes/              # questions, upload, admin, payments, ai, users
...
| GET | `/api/admin/questions` | admin | Moderation list (+ `?search`, `?status`, `?institution`, `?institutions=true`) |
...
| POST | `/api/payments/webhook` | signature | Paystack webhook (raw body HMAC-verified) |
| GET | `/api/user/subscription` | ✅ | Current user's subscription |
| GET | `/api/users/:userId/uploads` | — | Questions uploaded by a user |
| POST | `/api/ai/process-document` | — | Run the Genkit document flow (data URI or Google Drive URL) |

The upload pipeline: file → Supabase Storage → `question_uploads` record → async OCR
(Hugging Face or mock) → Genkit metadata extraction → `questions` row with `status: 'pending'`
→ admin approval → public question bank.
